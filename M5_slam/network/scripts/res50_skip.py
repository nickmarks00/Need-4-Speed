...
class Resnet50Skip(nn.Module):
    def __init__(self, args):
        self.args = args
        super(Resnet50Skip, self).__init__()
        res50 = models.resnet50(pretrained=False)
        self.res50_backbone = nn.Sequential(*list(
            res50.children())[:-6])
        ...
        self.top_conv = nn.Sequential(
            nn.Conv2d(in_channels=2048, out_channels=128, kernel_size=1),
            nn.ReLU())
    ...
    def forward(self, img):
        ...
        #p1 = self.upsample_add(p2, self.lateral_conv4(c2)) 
        #p0 = self.upsample_add(p1, self.lateral_conv5(c2)) 
