...
def calibrateWheelRadius():
    ...
    ##########################################
    wheel_velocities_range = [40] #range(20, 36, 15)
    delta_times = []
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        # In this case, wheel vel is given in ticks/time and is a representation of omega (W)
        #we know dt, thus we can get dx/dt which is v in m/time
        #We know v = Wr, now get r in terms of m/ticks by substituting and solve for r. V/W becomes (m/time)/(ticks/time) which cancels time and produces a m/ticks output 
        # TODO: replace with your code to compute the scale parameter using wheel_vel and delta_time
        V = 1/delta_time
        W = wheel_vel
        scale += V/W #get the result of each scale and sum
    scale = scale/num #get the average scale
...
def calibrateBaseline(scale):
    ...
    # Feel free to change the range / step
    ##########################################
    wheel_velocities_range = [10] #range(30, 60, 10)
    delta_times = []
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        ...
        baseline += scale/(np.pi/delta_time)*wheel_vel 
    baseline = baseline/num #average it out
