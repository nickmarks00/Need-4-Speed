...
class EKF:
    ...
    def predict(self, raw_drive_meas, lv, rv):
        ...
        self.x_hat = self.get_state_vector()

        Q = self.predict_covariance(raw_drive_meas, lv, rv)
    ...
    def predict_covariance(self, raw_drive_meas, lv, rv):
        n = self.number_landmarks()*2 + 3
        Q = np.zeros((n,n))
        ...
        if lv and rv == 0: # Stopped
            ...
            cov = 0.01 # Higher covariance since turning is less consistent

        Q[0:3,0:3] = self.robot.covariance_drive(raw_drive_meas) + cov*np.eye(3)
        return Q
